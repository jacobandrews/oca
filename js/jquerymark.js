...
let ganadores = 0; // Numero de ganadores
...
function aplicarConfiguracion() {
    ...
    for (let i = 0; i < numJugadores; i++) {
        ...
        let jugador = {
            ...
            color: color,
            turno: false,
            jugando: true /** Mark - Indicador para ver si el jugador está jugando todavia o si ganó y no le va a tocar ningún turno*/
        };
                ...
                <div class="col-md-4">
                    ...
                    <div class="dropdown">
                        <button class="btn btn-secondary dropdown-toggle" type="button" id="colorJugador${i + 1}" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            ${color ? `<div class="ficha" style="background-color: ${color}; width: 20px; height: 20px; display: inline-block; vertical-align: middle; margin-right: 10px;"></div>` : 'Color'}
                        </button>
                        <div class="dropdown-menu" aria-labelledby="colorJugador${i + 1}" style="width: auto;">
                            ${coloresDisponibles.map(c => `<a class="dropdown-item" href="#" onclick="seleccionarColor('${c.valor}', ${i + 1})"><div class="ficha" style="background-color: ${c.valor}; width: 20px; height: 20px; display: inline-block; vertical-align: middle; margin-right: 5px;"></div></a>`).join('')}
                        </div>
                    </div>
                </div>
            </div>
...
function seleccionarColor(color, jugador) {
    $(`#colorJugador${jugador}`).html(`<div class="ficha" style="background-color: ${color}; width: 20px; height: 20px; display: inline-block; vertical-align: middle; margin-right: 10px;"></div>`);
...
function actualizarColoresDisponibles() {
    ...
    for (let i = 0; i < numJugadores; i++) {
        ...
        let dropdownItems = coloresDisponibles.map(c => `<a class="dropdown-item" href="#" onclick="seleccionarColor('${c.valor}', ${i + 1})"><div class="ficha" style="background-color: ${c.valor}; width: 20px; height: 20px; display: inline-block; vertical-align: middle; margin-right: 5px;"></div></a>`).join('');
...
function iniciarJuego() {
    ...
    let controlesJugadores = `
        ...
        <div class="row justify-content-center mb-3 text-center">
            ...
            <div class="col-md-1"> <!-- Columna para el número del dado -->
                <strong>Nº</strong>
            </div>
        </div>
    `;
        ...
        controlesJugadores += `
            <div class="row mb-3 justify-content-center align-items-center"> <!-- Ajuste de alineación vertical -->
                ...
                <div class="col-md-2 text-center d-flex justify-content-center" id="btnTirarDado${i}"> <!-- Mostrar el turno y botón de tirar dado -->
                    <button class="btn btn-primary" onclick="moverFicha(${i})">Tirar Dado</button>
                </div>
                <div class="col-md-1 text-center" id="resultadoDado${i}"> <!-- Mostrar el resultado del dado -->
                </div>
            </div>
        `;
    }
...
function moverFicha(index) {
    ...
    resultadoDado.text(movimientos);
    ...
    // Actualizar la casilla actual del jugador
...
function actualizarBotonesTurno() {
    for (let i = 0; i < numJugadores; i++) {
        $(`#btnTirarDado${i}`).html(jugadores[i].turno ? `<button class="btn btn-primary" onclick="moverFicha(${i})">Tirar Dado</button>` : '');
    }
}
...
/** Mark - Función para la logica que se debe ejecutar cuando un jugador gana */
